import { existsSync, readFileSync, copyFileSync, readdirSync, rmSync, mkdirSync } from "node:fs";
import { join, resolve } from "node:path";
import sqlite3 from "sqlite3";
import { Database, open } from "sqlite";
import { version } from "rocketicons/package.json";
import { getContents, templateBuilder, write } from "./utils";

const OUTPUT_FILE = "icons/manifest.ts";
const SOURCE_SVGS = "../generator/svgs";
const SOURCE_MANIFESTS = "../../../icons";
const DEST_SVGS = "./src/app/data-helpers/svgs/";
const DEST_SQL_SVGS = resolve("./src/app/data-helpers/svgs/svgs.db");

let totalIcons = 0;
let totalCollections = 0;

const ManifestTemplate = `
// THIS FILE IS AUTO GENERATED
export const pkgVersion = "{0}";
export const totalCollections = {1};
export const totalIcons = {2};
`;

const hasChanges = () =>
  readFileSync(`${SOURCE_SVGS}/.lock`).toString() !==
  readFileSync(`${DEST_SVGS}/.lock`).toString();

const handleFiles = async (files: string[], collection: string, db: Database) => {
  const {
    default: {
      manifest: { name, license, projectUrl, licenseUrl, icons }
    }
  } = await import(join(SOURCE_MANIFESTS, collection, "manifest"));
  const dataIcons = Object.values(icons) as { id: string; name: string; compName: string }[];
  await db.run("INSERT INTO collections VALUES (?, ?, ?, ?, ?, ?)", [
    collection,
    name,
    license,
    projectUrl,
    licenseUrl,
    dataIcons.length
  ]);
  for (const filename of files.filter((filename) => filename.endsWith(".json"))) {
    const { name, compName } =
      dataIcons.find((icon) => icon.id === filename.replace(".json", "")) || {};
    const contents = getContents(join(SOURCE_SVGS, collection, filename), true);
    await db.run("INSERT INTO svgs VALUES (?, ?, ?, ?)", [
      join(collection, filename),
      name,
      compName,
      contents
    ]);
    totalIcons++;
  }
};

const generator = async () => {
  if (!existsSync(`${DEST_SVGS}/.lock`) || hasChanges()) {
    rmSync(DEST_SVGS, { recursive: true, force: true });
    const collections = readdirSync(SOURCE_SVGS, { withFileTypes: true });
    mkdirSync(DEST_SVGS, { recursive: true });
    const db = await open({ filename: DEST_SQL_SVGS, driver: sqlite3.cached.Database });
    await db.exec(
      "CREATE TABLE svgs (id STRING PRIMARY KEY, name STRING, compName STRING, svg TEXT)"
    );
    await db.exec(
      "CREATE TABLE collections (id STRING PRIMARY KEY, name STRING, license STRING, projectUrl STRING, licenseUrl STRING, totalIcons INTEGER)"
    );
    for (const { name } of collections.filter((entry) => entry.isDirectory())) {
      const files = readdirSync(join(SOURCE_SVGS, name));
      await handleFiles(files, name, db);
      totalCollections++;
    }

    await db.close();
    await write(
      OUTPUT_FILE,
      templateBuilder(ManifestTemplate, version, `${totalCollections}`, `${totalIcons}`)
    );
    copyFileSync(`${SOURCE_SVGS}/.lock`, `${DEST_SVGS}/.lock`);
  }
};

generator();
