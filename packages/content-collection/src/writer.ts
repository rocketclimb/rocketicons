...
function createArrayConstName(name: string, notArray = false) {
  let suffix = name.charAt(0).toUpperCase() + name.slice(1);
  return !!notArray ? suffix : "all" + pluralize(suffix);
}

type DataFileCollection = Pick<
  TransformedCollection,
  "name" | "documents" | "notArray"
>;
...
type JavaScriptFileConfiguration = {
  collections: Array<Pick<TransformedCollection, "name" | "notArray">>;
};
...
async function createJavaScriptFile(
  ...
  const collections = configuration.collections.map(({ name, notArray }) =>
    camelcase(createArrayConstName(name, notArray))
  );
...
import { GetTypeByName } from "@rocketclimb/content-collections";
`;
  ...
  for (const collection of collections) {
    ...
    if (collection.notArray) {
      content += `export declare const ${camelcase(
        createArrayConstName(collection.name, true)
      )}: ${collection.typeName};\n`;
    } else {
