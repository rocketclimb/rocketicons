...
- [ ] Extend the release pipeline to publish `@rocketicons/utils`, `@rocketicons/tailwind`, `@rocketicons/toolkit`, and `@rocketicons/mcp` in dependency order before the `rocketicons` package. **Partial:** Cut Release now generates all collections automatically before workspace tests and verifies every generated export and catalog record; build failures propagate to CI. The cut and publish workflows are updated, but a live release and npm trusted publishing configuration for the scoped packages remain unverified; this local host has no npm publish credentials.
